...
class Wallet extends EventEmitter {
  ...
  async e2m_send(options, passphrase) {
    const unlock = await this.fundLock.lock();
    try {
      return await this._e2m_send(options, passphrase);
    } finally {
      unlock();
    }
  }
  ...
  async _e2m_send(options, passphrase) {
    const mtx = await this._e2m_createTX(options, true);
    return this.sendMTX(mtx, passphrase);
  }

  async _e2m_createTX(options, force) {
    ...
    await this._e2m_fund(mtx, options, force);

    return this.finalize(mtx, options);
  }
  
  async _e2m_fund(mtx, options, force) {
    const unlock = await this.fundLock.lock(force);
    try {
      return await this._e2m_fill(mtx, options);
    } finally {
      unlock();
    }
  }

  async _e2m_fill(mtx, options) {
    if (!options)
      options = {};
