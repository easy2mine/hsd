...
class RPC extends RPCBase {
  ...
  async sendMany(args, help) {
    ...
    const valid = new Validator(args);
    let name = valid.str(0, '');
    let changeAddress = valid.str(1, '');
    const sendTo = valid.obj(2);
    const minconf = valid.u32(3, 1);
    const subtract = valid.bool(5, false);
    ...
    const tx = await wallet.e2m_send(options);
